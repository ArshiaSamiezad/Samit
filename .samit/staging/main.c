...
int run_commit(int argc, char *argv[], int level)
{
    ...
    if (level == 0)
    {
        ...
        {
            printf("Hook failure is detected. Press 1 to continue:\n");
            int go;
            scanf("%d", &go);
            if (go != 1)
            {
...
int run_hooks(char working_dir[], char working_name[])
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        {
            if (strcmp(file_suffix, "txt") && strcmp(file_suffix, "c") && strcmp(file_suffix, "cpp"))
            {
                chdir(main_dir);
                chdir(".samit/hooks/applied");
            ...
            int is_failed = 0;
            while (fscanf(todo_file_check, "%s", string_check) != EOF)
            {
                // text
        ...
        {
            if (strcmp(file_suffix, "txt") && strcmp(file_suffix, "c") && strcmp(file_suffix, "cpp") && strcmp(file_suffix, "mp4") && strcmp(file_suffix, "wav") && strcmp(file_suffix, "mp3"))
            {
                chdir(main_dir);
                chdir(".samit/hooks/applied");
...
int run_precommit(int argc, char *argv[], int level)
{
    ...
    if (argc == 2)
    {
        ...
        struct dirent *entry;
        DIR *dir = opendir(destination_file);
        char cwd_ba[1000];
        char tmp_dest_file[MAX_FILENAME_LENGTH];
        strcpy(tmp_dest_file, destination_file);
        // printf("temp is %s\n",tmp_dest_file);
